...
public class ChartWaterActivity extends AppCompatActivity {
    ...
    private void loadChartData() {
        ArrayList<BarEntry> waters = new ArrayList<>();
        ArrayList<String> dates = new ArrayList<>();

        Cursor cursor = database.GetDataBase("SELECT SUM(amount), DATE(timestamp) FROM water_table WHERE DATE(timestamp) = DATE('now', 'localtime') GROUP BY DATE(timestamp)");
        int index = 0;

        if (cursor.moveToFirst()) {
            do {
                int water = cursor.getInt(0);
                String date = cursor.getString(1);

                waters.add(new BarEntry(index, water));
                dates.add(date);
                index++;
    ...
    private void updateChartData(String timePeriod) {
        ...
        switch (timePeriod) {
            case "day":
                query = "SELECT SUM(amount), DATE(timestamp) FROM water_table " +
            ...
            case "week":
                ...
                break;
            case "month":
                query = "SELECT SUM(amount), strftime('%Y-%m', timestamp) FROM water_table " +
            ...
            case "6month":
                ...
                break;
            case "year":
                query = "SELECT SUM(amount), strftime('%Y', timestamp) FROM water_table " +
    ...
    private void updateChart(ArrayList<BarEntry> waters, ArrayList<String> dates) {
        BarDataSet dataSet = new BarDataSet(waters, "Tổng lượng nước (ml)");
        ...
        yAxis.setAxisMaximum(10000f); // Đặt giá trị tối đa
